...
export const CampaignVaultPlugin: Plugin = async ({ client }) => {
  ...
  return {
    ...
    "tool.execute.after": async ({ tool, sessionID }, output) => {
      if (!matchesWorldStateTool(tool)) return;
      ...
      if (isStartSessionTool(tool)) {
        ...
        if (info) campaignInfoBySession.set(sessionID, info);
      }
