...
- [x] T2 **Scene events, pressure, chrome** (A4 travel events dropped and capped at 4; A6/F5 one party-only vulnerability line, name-based weapon match, example in `help faq`; A5: exit wire view `LocationExitView` (no `oneWay:false`, no zero cost), `dangerModifier`/`descriptionTruncated` omitted at default, scene events without `involved`/`locationId`/default importance and without ids for auto-logged SceneCommit narratives, no `proficiencyBonus` in the stat line, rounded initiative tension, `partyFingerprint` only when the client's copy is stale, `seededNpcIds`/`knownCharacterIds` removed once rosters name everyone, delta scenes with no news dropped). Not done: generated exit descriptions; nothing generates them (the scratch replay authors "Path to X").
- [x] T3 **Beat trims**: shorter novelty hints, no hint for Travel/Arrival events, one novelty hint per beat (C1); `tokensEst` removed (D1); no "Event logged (id)" line (committedIds has it); `rateLimitTokensRemaining` only at ≤10 tokens; envelope "Committed N changes."; the combat/status reminder no longer fires on a plain ruleset_action. Round 4 item 1 done: `GuidanceOrchestrator` records delivery in `GuidanceLedger`, `start_session` clears it (each hint once per session). `EventFollowUpAdvisor` hints ("no 'activity' commit found ...") also go out once per kind per session.
...
- [x] T5 **Travel** (F1 grouping + prorated buckets, F2 one clock advance per group, F3 "Arrived"; separation: optional `hazard` on `travel` ("dense fog"), or night in the wild, never on legs under an hour; a rare roll (12% / 4%) by the group's first traveler loses one non-PC follower, who stays at the origin with an explicit `SEPARATED: ... is back at <origin>` report).
- [x] T5b **Remove POIs** (done: startup migration `MigratePointsOfInterestToFixtures`, `world_build` shim `PoiFixtureShim` (details → fixture items, names ignored), POI fields gone from `location_update`, `LocationDetailView`, `get_entity detailPoi`, the POI/flavor/sparse-crowd nags and the materializer; skills, help and prompts updated. The three legacy `Location` fields remain only for the migration and can be deleted once every deployment has run it. A5 scene-chrome trims still open). Original scope:
...
- [x] T5c **Hidden content and hazards** (done):
  - `hidden` + `discoverDc` + `intent` on exits, `hidden` + `discoverDc` on items and item details, `Hazard` (name, trigger enter/take, detect/disarm DC, effect, save DC/ability, intent; engine-set detected/disarmed/spent, optional re-arm) on exits, items and locations. Authoring: world_build/upsert fields, `item_update` hidden/discoverDc/hazard, `location_update.addHazard`, `upsertItemDetail` hidden/discoverDc; the auto-created reverse of a secret exit is secret too.
  - Off the wire: hidden exits (`LocationDetailView`), scene items, container contents, NPC gear in scenes and summaries, hidden detail names, `search_world` items. Query filters use `Hidden != true` so documents saved before the field existed still match (Raven skips them for `!Hidden`; test `HiddenFilter_KeepsItemsStoredBeforeTheFieldExisted`).
  - First-visit DM-only line: `fullScene.dmOnly` lists a location's secrets with DCs and intents (plus fixture details with an `intent`), once per session.
  - Engine-resolved: an Investigation/Perception skill check reveals what its total meets (`FOUND:`); arriving party members' passive Perception does the same (`NOTICED:`); a live, unspotted trap fires on its trigger (`HAZARD: ... Resolve now: ruleset_action SavingThrow (...)`), a spotted one gets a "known trap" line; a check with `parameters.disarm` = the trap's name disarms it (a miss by 5+ sets it off); using a secret exit reveals it; taking a concealed item reveals it.
  - Guidance: `dnd-world-building` step 4, `dnd-exploration` search section, `help world-building` step 6.
  - Not built (by design): the existence oracle waits for the T9 played test. The hazard effect is reported for the DM to resolve with a ruleset_action rather than auto-rolled.
- [x] T6 **Need-driven responses** (done). Ledger on `TurnCursor` (`DeliveredCardHashes`, `DeliveredContextKeys`, `BriefedLocationIds`), cleared only on a real context loss: start_session (`LedgerResetPending`), a client `forceFullReseed`, fingerprint drift, the periodic reseed (not on advance_world's forced Full). Rosters: `presentNPCs` stays the wire list (get_entity and 60+ assertions depend on it) but full-scene rows are id/name/activity/mood + an 80-char note until the card is sent; delta rows keep only this turn's changes and the live initiative reading. Cards (`cards[]`: mood, traits, wants, fears, stance, notes, looks, AC/level, gear, pressing needs ≥60, own need notes, ≤2 memories that match the beat or are Core/urgent) once per session: on arrival for spotlight NPCs (plot/quest-linked, about to act, keepAlive, an opinion of the party, companions) and on first involvement; the hash ignores mood, stance scores and item counts. `npcs[]`: requested, involved or acting NPCs only; no behavioralSummary; a routine initiative pick becomes one "Likely to act next" context line. Context contributors (`IContextContributor`, plugin `IPluginContextContributor`, ~800-char budget with an overflow pointer): topic-matched memory recall (≤2), relationship tier crossing (replaces the tier-crossing Full reseed), pressing need, trade (party gold, the merchant's wares), stealth (passive Perception here), search (what the room holds), spell (slots left), quest link. First-visit description (a revisit says "(described earlier this session)"). Prompts/skills/help: approach beat on first contact, rosters/cards/context. opencode plugin: `forceFullReseed` on the first take_turn after `session.compacted`. Not built: a combat roster line (the `combat` tool already returns the combatant roster every call).
- [x] T7 **Response budgets** (`ResponseBudgetTests`, realistic NPCs with traits/wants/fears/notes/memories/gear): 4-NPC arrival 2806 chars (ceiling 3300; the 2.5k target assumed ~120-char synthetic cards, realistic ones are ~700), first-contact beat 1544 (1900), repeat beat 344 (450), after rest 1701 (2000, one-time guidance included). `TokenBudgetMeasurementTests` delta/full ratio gates reset to 1.0: the full snapshot is now a lean roster, so a delta can at best match it; the absolute ceilings and `ResponseBudgetTests` carry the size guarantee.
...
- [x] T8 (final, after T6/T5c/T7) Fresh scratch DB, take_turn chars per session. Plain seed (comparable with the baseline): S1 26.9k, S2 29.2k, S3 31.5k, i.e. **-69% / -73%** against 86.2k / 106.5k. With realistic NPCs (`RICH=1`): S1 29.9k, S2 31.3k, S3 33.8k. What remains in S1 (rich): fullScene 8.7k over 7 arrivals (roster 4.4k, location 3.1k), cards 4.3k (once per session each), novelty lines 2.4k, fullNpcContext 2.2k (an explicit pull), worldStateDelta 1.6k.
