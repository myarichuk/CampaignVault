...
public class CampaignToolsTests : IClassFixture<RavenDBFixture>
{
    ...
    public async Task GetScene_Emits_SuggestedCommitExamples_ForInterruptedTravel_AndPopulatesSummaries()
    {
        ...
        using (var session = _fixture.Store.OpenAsyncSession())
        {
            await repo.UpsertLocationAsync(_fixture.CreateCampaignSession(session, TestCampaignDefaults.Slug), new LocationUpsertRequest { Id = locId, Name = "Road" });
    ...
    public async Task GetScene_EmitsMemoryDecayPressure()
    {
        ...
        using (var session = _fixture.Store.OpenAsyncSession())
        {
            ...
            await repo.UpsertLocationAsync(_fixture.CreateCampaignSession(session, TestCampaignDefaults.Slug), new LocationUpsertRequest { Id = locId, Name = "Room" });
            ...
            await repo.UpsertCharacterAsync(_fixture.CreateCampaignSession(session, TestCampaignDefaults.Slug), c);

            var t = await repo.GetTimeAsync(_fixture.CreateCampaignSession(session, TestCampaignDefaults.Slug));
    ...
    public async Task GetScene_EmitsOpportunisticPressure()
    {
        ...
        using (var session = _fixture.Store.OpenAsyncSession())
        {
            await repo.UpsertLocationAsync(_fixture.CreateCampaignSession(session, TestCampaignDefaults.Slug), new LocationUpsertRequest { Id = locId, Name = "Alley" });
