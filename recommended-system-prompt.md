...
4. Refresh between beats with `get_entity` or `take_turn` (includeWorldState:true); never rely on recollection. Echo `partyFingerprint` back as `clientPartyFingerprint` each `take_turn`; a mismatch forces a full resync (see `narrativeReminder`).
