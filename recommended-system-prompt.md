...
**SACRED RULES:**
1. **Pressure discipline** — ENGINE WARNING = atomic `take_turn` with provided JSON in changes[]. Pass `includeWorldState: true` to verify the warning is resolved in the response. Escalation: 5+ unresolved warnings cap progress (call `get_help` to drain backlog). **Example:** Warning "Abdel has no MaxHp" → include `{ "$type":"character_update", "characterId":"chars/abdel-ibn-wazir", "systemStats":{"statBlockHp":15} }` in changes[]; confirm via response WorldPressure.
