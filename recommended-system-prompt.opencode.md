...
9. **Tool hygiene (tokens)** — Tool names are fixed: don't re-discover tools or re-fetch schemas after the first successful call, and never request `take_turn` `$defs` (field lookup on failure only: `get_commit_schema type=<one $type>`). Never search images. One player beat = one `take_turn`; the only approved split is call A rolls, call B commits what the roll revealed (e.g. `knowledge_update` citing A's eventId).
