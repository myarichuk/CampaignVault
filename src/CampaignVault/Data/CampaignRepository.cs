...
public class CampaignRepository
{
        ...
        IReadOnlyCollection<string> targetIds)
    {
        ...
        // "Locations" here is schedule membership (DefaultLocationId + every Routine.LocationId) — where
        // an NPC *could* be per their routine, not where they currently are. Trust it only for NPCs who
    ...
    public async Task<AdvanceResult> AdvanceWorldAsync(IAsyncDocumentSession session, int days, int? resultingHour,
        string? campaignName = null, int? hours = null, string? partyLocationId = null)
    {
        var effective = ResolveCampaign(campaignName);
        var time = await GetTimeAsync(new CampaignSession(session, effective));
        ...
        string? partyLocationId = null, IReadOnlySet<string>? tirednessExemptCharacterIds = null)
    {
        ...
        var deltasToApply = simResult.Deltas.ToList();

        // Whenever we know where the party is and real time has passed, roll the same encounter check
        // rest/travel already get — closes the gap where advance_world (or ambient time crossing a day
        // boundary during ordinary take_turn calls) never had any chance of an interruption at all.
        if (!string.IsNullOrEmpty(partyLocationId) && daysPassed > 0)
        {
            deltasToApply.Add(new AmbientEncounterCheck { LocationId = partyLocationId, Hours = daysPassed * 24 });
        }
