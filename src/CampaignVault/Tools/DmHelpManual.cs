...
- Use `$type: ""item""` (`itemId` + `toHolderId`) to move an *existing* item to a new holder (a character, location, or container item) — e.g. a PC hands off a torch, or loot gets dropped in a location. Not for creating or editing item properties; use `world_build`/`item_update` for that.
- Use `item_update`'s `upsertItemDetail` to track persistent, granular details on an item — scratches, stains, secret compartments, custom pockets — that should survive across sessions (unlike `TagsToAdd`, which is for temporary labels). Pass `id` if you already know it (from a prior commit response or `get_item`) to update that exact detail; omit it and the engine resolves by semantic similarity, falling back to creating a new one. `intent` is DM-only narration/mechanics guidance (never shown to players) — e.g. a suggested DC or discovery condition. List `participants` (with `role: ""Caused""` or `""Witnessed""`) to push a memory into those characters' psychology automatically. Use `retireItemDetailId` to soft-retire a detail once it's no longer true (compartment emptied, stain cleaned) — it keeps the record (so any memory referencing it stays resolvable) rather than deleting it.
