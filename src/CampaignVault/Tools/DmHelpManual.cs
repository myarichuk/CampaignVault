...
6. **secrets (0-2, only where a plot thread, quest or NPC secret lives)** — a secret way is an exit with `hidden`, `discoverDc`, `intent`; a concealed object an item with `hidden`, `discoverDc`; a secret on a fixture an `itemDetails[]` entry with `hidden`, `discoverDc`, `intent`; a trap a `hazard` (`name`, `trigger` enter/take, `detectDc`, `disarmDc`, `effect`, `saveDc`, `saveAbility`) on an exit, an item, or the location's `hazards`. They stay off the scene; the engine reveals them when a Perception/Investigation check or passive Perception meets the DC (`FOUND:`/`NOTICED:`), fires traps on their trigger (`HAZARD:`), and disarms on a check with `parameters.disarm` = the trap's name. You get them once per session as `fullScene.dmOnly` on arrival.
