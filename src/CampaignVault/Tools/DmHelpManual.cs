...
6. **Call `advance_world`** for genuinely uneventful travel, rests, or downtime skips.
7. **Never roll dice yourself** (mentally, or via an external script/tool) for anything that should be recorded — not just attacks. Ambient Perception on arrival, Investigation, Stealth, a passive save: all of it goes through `take_turn`'s `ruleset_action` change with `actionType: ""SkillCheck""` (or `SavingThrow`/`ContestedCheck` as appropriate), the same $type used for combat. It is the engine's only dice roller, in or out of combat. Example: `{""$type"":""ruleset_action"",""characterId"":""chars/lyra"",""actionName"":""Perception"",""actionType"":""SkillCheck"",""actionCategory"":""Survival"",""parameters"":{""dc"":""12""}}`.
...
3. Pass `campaignName: ""dragon-heist""` explicitly on every call to `get_entity`, `take_turn`, etc.
...
**Party roster:** Tag human PCs with `isPc: true` and NPC companions with `isPartyCompanion: true` (mutually exclusive; both require a campaign slug). `start_session` returns only those flagged characters as the party — not ambient `keepAlive` NPCs; mid-session, refresh the roster with `take_turn` (`includeParty: true`). Combat accepts canon entities (no `CampaignName`) plus campaign-tagged combatants; it rejects entities tagged for a different slug.

## The Golden Rule: The One-Door Principle

A **new entity or full replace** always goes through `world_build` (batch: characters, locations, items, factions, quests, rumors, plotThreads, creatures, spells, feats, lore, needDescriptors) — there is no `_create` `$type` in take_turn's changes for these. See `get_help topic=world-building` for the full guide and a copy-paste example.

A **change to something that already exists** (numeric/state deltas, tag/exit add-remove, progress) goes through `take_turn`'s changes[]. During play, strongly prefer take_turn changes (especially `activity`) for everything except creating a brand-new entity.

## Narrative Focus & Event Importance

`NarrativeFocus` is a free-text tag list on the campaign describing the kind(s) of story it tells (e.g. `[""political intrigue""]`, `[""dungeon crawl""]`, `[""horror investigation""]`). There is no server-side genre→importance matrix — these tags exist purely to steer **your** judgment as DM when you set `importance` on an `event` change. Campaigns evolve; update the tags whenever the story's center of gravity shifts.

**Setting it:** pass `narrativeFocus` to `create_campaign`, or commit `{ ""$type"": ""campaign_update"", ""narrativeFocus"": [...] }` via `take_turn` at any point afterward (this replaces the full tag list, not just adds to it). `start_session` echoes the current tags back in its campaign block.

**Choosing `importance` on `event` changes:** every event has an `importance` of `Trivial`, `Important`, or `Core`. If omitted, the engine defaults it by category (`Betrayal`/`Discovery`/`Combat`/`Arrival` → `Important`; engine bookkeeping categories like `Departure`/`Timeskip`/`Simulation` → `Trivial`). But the *same raw happening* should often get a different importance depending on what this campaign is actually about — you should set `importance` explicitly whenever the default would misjudge it:
...
## Core Gameplay Loop
1. **Start of Session**: Call `start_session` (with party location if known) to sync recap, time, rumors, events, char distress, **and WorldPressure**.
2. **Exploration**: Call `get_entity` with the location id on entry (partyPresent:true). **Immediately action any ENGINE WARNING / NARRATIVE PROMPT in the WorldPressure** (use the exact JSON provided).
3. **Action & Consequence**: Narrate vividly to players. At end of beat (or when something should persist), call `take_turn` with the array of changes — the response echoes fresh summaries for every touched entity, so there is no query→commit→query loop. Use `activity` liberally to keep sim in sync.
4. **Time Skips / Travel**: `advance_world` (triggers needs, rumor decay, schedule eval, **TransientEvictionRule** for flavor NPCs).
5. **Deep NPC**: `get_entity` with the `chars/...` id (full psychology, needs, memories, behavior synthesis) — or bundle it with a mutation via `take_turn`'s `fullDetailCharacterId`.

**Golden Rule:** If you just narrated something that should ""exist"" next time the party returns or is referenced, persist it via `take_turn` (or `world_build` for a new entity). If it's pure color, use PointsOfInterest + AmbientCrowd (lightweight, no docs created until you decide to promote).

## Time During a Scene (not just travel/rest)

Ordinary scenes take in-game time too — a long interrogation or a late-night talk isn't free just because nobody called `advance_world`. Any change (except `rest`/`travel`, which have their own hour fields) accepts an optional `minutesElapsed`: summed across the batch, it nudges hunger/thirst/tiredness immediately, no day-boundary or `advance_world` required. Set it whenever a beat represents more than a few seconds — a quick exchange ≈2-5 min, a tense negotiation or a multi-hour talk ≈60-180 min.

For tense or crowded scenes (interrogation, stakeout, a heated negotiation — not combat-only), also include `scene_interrupt_check` after the beat (not every line) to let the engine roll whether someone from the ambient crowd interrupts. Cooldown: once per location per day. See `get_help topic=combat` or `get_commit_schema` for the full field list.
...
- **Change type reference**: call `get_help topic=commit-enum`
- **Full MCP tool catalog**: call `get_help topic=tools`
- **FAQ & laziness traps**: call `get_help topic=faq`

## Tools by Category

Call `get_help topic=tools` for the full catalog. The surface is deliberately small — each tool has exactly one job:

**Session & Exploration**: `start_session` (kickoff superset — once per session), `search_world` (fuzzy/semantic entity search), `recall_history` (ground-truth event search), `end_session`

**Mutation & Time**: `take_turn` (THE mutation tool: changes[] + narrative in, commit outcome + fresh state out; also serves pure state refreshes), `advance_world` (uneventful skips only)

**Combat & Rulesets**: `combat` (action: start | next | end | status — lifecycle only; attacks/spells go through take_turn's `ruleset_action`)
...
**System**: `get_help` (this), `get_commit_schema`
...
## take_turn (Universal Write + Bundled Refresh)
ALWAYS call at end of combat/conversation/discovery. Atomic array of `$type` mutations (in `request.changes`) plus a `narrative` summary. Mutations are processed atomically as a single database transaction, and the response echoes fresh summaries for touched NPCs/scenes — never re-query after a mutation.

- **Batch Size Guidance:** Individual calls are capped at a maximum of **50 changes**. Group all related mutations (e.g. travel, quest progress, HP updates, and activity updates) into a single batch to ensure consistency.
...
**Overnight/partial-day spans without full-on resting** (a long watch, waiting out a storm) that aren't a `rest` change and aren't a `travel`: use `advance_world`'s `hours` parameter (e.g. `hours: 8`) instead of computing `days`/`timeOfDay` by hand — the engine derives the resulting time from the current clock. But `advance_world` itself has no encounter mechanic at all; if the span carries real risk, commit a `rest` (camping) or `travel` (moving) instead so the risk actually gets rolled.

**Ambient interrupts happen automatically, no separate change needed.** Any ordinary batch carrying `minutesElapsed` (a long search, an interrogation, a stakeout) is itself automatically eligible for an interrupt roll — you don't need to also include `scene_interrupt_check` for this to work. It only fires where `location.DangerModifier > 0` or the location's `ambientCrowd` reads as dense; a location with neither never rolls (a locked, empty safe-house stays quiet). It's skipped entirely if the batch already contains an explicit `rest`/`travel`/`scene_interrupt_check`, or during active combat. If it fires, the take_turn response's summary includes an `AMBIENT INTERRUPT` line with a transient NPC/situation to resolve — same non-forced-combat design as the other three mechanisms. Set `dangerModifier` deliberately on your locations (see seeding order above) — it's the only lever that controls whether this happens at all.
...
A scene's `TurnIntentCharacterId` / an NPC's `TurnIntent` (in `get_entity` full views or take_turn full-detail; advisory only, never a hard gate — see recommended system prompt) aren't static flavor text; they're computed from state that time and play actually move:

- **Needs pressure**: `social_drive` is a tracked need like hunger/thirst/tiredness — it rises the same way, via ordinary `minutesElapsed` on scene changes and `advance_world`/`rest`/`travel` (see ""Time During a Scene"" above). An NPC whose `social_drive` climbs high enough becomes an initiative candidate wanting attention — this is why a long stretch of scenes with no interaction for a present companion can make them start ""wanting"" something, without you doing anything special to cause it.
...
(You used ambient flavor + PoIs implicitly via narration. No persistence yet - correct for pure color.)

Later, party talks to the bard or barman engages:
- Call `get_entity(""locations/rusty-nail"", partyPresent: true)` first (authoritative state).
...
- If they just drink and leave: no persistence needed for the 12 unnamed sailors. Engine will GC any you did transiently create if area goes cold.

## Travel, Faction, Quest & Rumor Batch

When the party resolves a rumor about a rebel smuggler by betraying them to the city watch, batch all the consequences in one `take_turn`:
[
...
This safely moves the party (with time + fatigue), updates the quest, modifies standing with two factions, resolves the active rumor, moves the smuggler NPC into jail with a new activity, and logs a narrative event in a single atomic database operation — and the response echoes the fresh state of everyone involved.

## Quest + Faction + Rumor Lifecycle (Full Arc)

A complete arc — from seeded rumor through investigation, faction reaction, and resolution — spans several take_turn calls.

**Beat 1 — Seed the thread (tavern, session start):**
Bram the barkeep mentions the Nightshade gang has been raiding river barges. Create the rumor and the quest hook via `world_build`, and flag Bram as the quest giver:
...
Then advance the quest and record the discovery via `take_turn`:
[
...
1. **take_turn an event** — category `Discovery`, recordingMode `Deliberate`, importance `Core` (this is a deliberate, load-bearing act).
2. **world_build** — type `Wilderness`, connectedFromLocationId set to the location they came from (auto-links with two-way exits).
3. **take_turn a knowledge_update** — on the recording PC, recordingMode `Deliberate`, importance `Core`, relatedEntityIds = [new location id], sourceEventIds = [event id from step 1].
...
(A separate `location_update` with `materializePointOfInterest`/`poiDetails` targeting the same location does the same thing and still works — the inline fields on `activity` are just the one-change version of the exact same mechanism.)
Ask yourself the same ""will this be referenced again"" test as any other change: a hidden killer on the loose, a stash, an ambush risk, a tracking check DC — all of that hinges on these details existing somewhere queryable. Skipping the PoI here isn't ""Schrödinger's World flavor,"" it's losing state the story is about to depend on. If the party never returns and nothing hinges on it, a plain `activity` with no PoI is fine — judge by stakes, not habit. If you do skip it and the batch also contains an Important/Core event referencing that location, take_turn's response will remind you.
...
**Flavor without bloat**: When narrating a crowded tavern, a bustling market, rats in a cellar, or ""a bard playing a lute in the corner"", **do not** immediately `world_build` 20 people. Instead:
  - On initial `world_build` or via `location_update`: populate `pointsOfInterest` (light list of strings returned in scene views) and/or `ambientCrowd` (string hint, e.g. ""8-15 rough sailors and dockworkers"").
...
**Dead-ends / broken maps**: the scene view will nag with ready `location_update` + `addExit`. Use it.

**Hallucinated locations**: fetching a bad location ID never throws. Returns stub + strong ENGINE WARNING with ready `world_build` JSON (including connectedFrom suggestion). Paste it.
...
*Combat vs manual: ruleset resolvers automatically establish and clear mechanical engagements (grappling, escape) via `ruleset_action` contested checks. For unresolved non-combat beats (hugs, tending wounds, intense confrontations), include `engagement_relation` yourself — otherwise scene pressure will nag you and Hard engagements block `travel`.*

**Multi-party conversations (PC + companion + NPC):** `engagement_relation` is pairwise — use one row per speaker↔anchor pair, then log the beat. Either set `involved` explicitly on the `event`, or let the engine merge participants from `engagement_relation`, `spatial_position`, `activity`, and `ruleset_action` in the same batch.
[
...
All participants in `involved` are recalled by each speaker's NPC context (get_entity with the chars/ id).
...
`get_entity` for any of the four will recall the debate and who was in it — no PC involvement required.

**Commit hints:** after a `Conversation`/`Discovery`/`Betrayal` event, the engine may return advisory `Hint:` lines in the take_turn summary — e.g. flagging that no `activity` change covered a participant's body language, that no `knowledge_update` captured what was learned, that a co-committed `knowledge_update` doesn't set `sourceEventIds`, or that the beat reads as unusually novel/repetitive versus recent events. These never fail the batch; treat them as a second-pass checklist, not an error.
...
- **What an NPC believes** (subjective, may have drifted or be flat wrong): `get_entity` with the chars/ id → `Psychology.Memories`.
...
**After you see a pressure in any scene/world-state response, your *next* action should usually be a `take_turn` using the exact snippet provided (adapted with real IDs/names).** Then narrate the outcome. The engine will clear the pressure on subsequent reads.
";
...
**Opportunity attack example** (reaction when the goblin disengages past the fighter):
```json
[
  {
    ""$type"": ""ruleset_action"",
...
**Existing PCs:** `world_build` creates the full PC document (`isPc: true`). During play use `activity` + `travel` to move PCs — do NOT re-`world_build` them to move them (the engine merges but it's the wrong tool for a routine move). The party roster comes back from `start_session`, or mid-session via `take_turn` with `includeParty: true`.
...
Use `status` inside `take_turn` to add effects, including mechanical modifiers.
...
Pressures appear in **every** world-state-carrying response — `start_session`, scene fetches via `get_entity`, `take_turn` (with `includeWorldState: true`), and `advance_world` (in the ToolResult.WorldPressure array, and also embedded in some views).
...
- **Simulation / character / rumor pressures**: Aging unresolved, dying PCs/NPCs, desperate needs, etc. Many now include mini example change snippets.

**Never ignore them.** The next scene fetch after you fix will usually have fewer or none. If you keep seeing the same one, you skipped the change.

Additional pressures come from character distress contributors (HP, bad statuses, high needs) surfaced via world-state refreshes, plus rule narratives turned into SimulatorEvents on advance.
...
Each pressure surfaces with a brief explanation and often includes **suggested change JSON** that you can copy-paste and adapt. **Use these immediately** — they're your co-DM's way of flagging world state that needs action.
...
- Set `climateZone` on `world_build` (Arctic, Tundra, Temperate, Desert, Tropical, Alpine, Subterranean). Zones inherit from parent locations; scene views report current ambient temperature and time of day.
...
- Ambient items (provisions, scrolls) with `ambientExpiresAtDay` set decay over time. World-state refreshes nag when expiry passes; resolve via `item_update` with a fresh expiry, `archive_entity`, or `item` transfer to a character.
...
- When narration establishes how a PC looks (bloodied, wanted, disheveled, unarmed), persist it via `character_update` with `appearanceOverride` and `tagsToAdd` (e.g., `[""bloody"", ""wanted"", ""disheveled""]`). In crowded or opportunistic-faction scenes, the scene view scores these tags and may nudge a crowd reaction.
...
**Optional roll:** after a tense beat (not every dialog line), include `scene_interrupt_check` with `riskModifier` (-50..+50, like `encounterRiskModifier` on travel) — omit `riskModifier` to auto-derive from tags. On success the engine promotes ONE transient from `ambientCrowd`; cooldown is one interrupt per location per day. Protective tags (`well_armed`, `escorted`, `uniform`) reduce the score.

## Faction Economic Demand

Factions have dynamic `EconomicDemand`. If a faction is desperate for an item the party is carrying (e.g. ""spell scrolls""), the scene view will pressure you to narrate merchants offering a premium or thieves attempting to steal them. Fulfill this naturally during roleplay!
