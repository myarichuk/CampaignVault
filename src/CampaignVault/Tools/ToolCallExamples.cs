        ...
        return new Dictionary<string, ToolCallExample>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["take_turn"] = new ToolCallExample
            {
                ...
                DeserializationHint =
                    ...
                    + "🚨 *** REQUIRED: '$type' IS NOT OPTIONAL *** 🚨 EVERY SINGLE change object MUST include a '$type' field. If even ONE change lacks '$type', the entire batch will be rejected with a deserialization error. Valid values: event, hp, engagement_relation, activity, status, status_remove, resource, rumor, quest_progress, plot_thread_progress, plot_thread_clue, location_update, character_update, travel, rest, level_up, schedule_change, item, item_update, item_equip, item_unequip, item_use, ruleset_action, faction_reputation, faction_state, scene_setup, scene_interrupt_check, spatial_position, memory_decay, knowledge_update, and 10+ more. See examples for correct structure. "
