...
| `get_scene` | Location, NPCs (with HeldItems + SystemStats), visible location items, rumors, ActiveCombat, pressures |
...
Supported `$type`s: `hp`, `item`, `item_update`, `status`, `statusremove`, `event`, `rumor`, `relationship`, `engagement_relation`, `spatial_position`, `need`, `attribute`, `mood`, `activity`, `ruleset_action`, `location_create`, `location_update`, `character_create`, `character_update`, `system_stats`, `knowledge_update`, `schedule_change`, `item_create`, `travel`, `rest`, `scene_interrupt_check`, `faction_create`, `faction_reputation`, `faction_state`, `quest_create`, `quest_progress`.
...
- **targetIds**: One ID per shot/target. For repeating weapons, list multiple merc IDs and set `attackCount` (or put `attackCount`/`rateOfFire` on the weapon item). Each target gets its own attack roll.
...
**Existing PCs:** `upsert_character` creates the full PC document. During play use `activity` + `travel` to move PCs — do NOT `character_create` them again (the engine merges but warns). Call `get_party` first.
