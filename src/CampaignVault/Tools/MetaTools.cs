...
public class MetaTools : IMcpServerTool
{
        ...
        string? type = null)
    {
        ...
        return Task.FromResult(new ToolResult<IReadOnlyList<CommitTypeSchema>>(
            true, schema,
            $"Returned {schema.Count} commit type schemas{(type != null ? $" for type '{type}'" : category != null ? $" for category '{category}'" : "")}. Side-effect types are marked hasSideEffects=true — do not duplicate their auto-mutations."));
    }
